...
function Setoran() {
  ...
  if (loading || !randomAyah) {
    return (
      <div className="min-h-screen mx-auto flex flex-col items-center justify-center bg-blue-50">
      ...
      <div className="flex-1 overflow-y-auto container max-w-2xl mx-auto px-4 pt-[70px] pb-20 scrollbar-hide">
        ...
        <div className="flex mb-4 bg-white rounded-xl p-1 shadow-sm">
          {['semua', 'setoran', 'murojaah'].map((tab) => (
            <button
              key={tab}
              className={`flex-1 py-2 text-sm font-medium rounded-lg capitalize ${activeTab === tab ? 'bg-blue-500 text-white' : 'text-gray-600'
                }`}
